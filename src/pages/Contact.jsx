...
export default function Contact() {
  ...
  const validate = () => {
    const errs = {}
    if (!formData.name.trim() || formData.name.length < 2) errs.name = 'Name is required (min 2 chars)'
    if (!formData.email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) errs.email = 'Valid email is required'
    if (!formData.subject.trim() || formData.subject.length < 3) errs.subject = 'Subject is required'
  ...
  return (
    <>
      <SEOHead
        ...
        description="Book Magnus the Illusionist for your event. Contact us to discuss your requirements and create a truly magical experience."
          ...
          <div className="container">
            <div className="text-center reveal" ref={useScrollReveal()}>
              ...
              <p className="section-subtitle" style={{ margin: '0 auto' }}>
                Ready to bring wonder to your event? Share the details and 
                Magnus will craft a performance tailored just for you.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="section">
          <div className="container">
            <div className={styles.contactGrid}>
                ...
                {status === 'success' ? (
                  <div className={styles.successMessage} role="alert">
                    ...
                    <p className={styles.successText}>
                      Thank you for reaching out. Magnus will personally review your enquiry 
                      and respond within 24 hours. Prepare to be amazed.
                    </p>
                    <button
                  ...
                  <form onSubmit={handleSubmit} className={styles.form} noValidate aria-label="Contact form">
                    ...
                    <div className={styles.formRow}>
                      ...
                      <div className="form-group">
                        <label htmlFor="service" className="form-label">Event Type</label>
                        <select
                          id="service"
                          name="service"
                    ...
                    <p className={styles.formNote}>
                      All enquiries are responded to within 24 hours. We take your privacy seriously.
                    </p>
                  </form>
                )}
              </div>
              ...
              {/* Info */}
              <div className={`${styles.infoSection} reveal-right`} ref={infoRef}>
                  ...
                  {contactInfo.map(({ icon, label, value, href }) => (
                    <div key={label} className={styles.contactInfoItem}>
                      ...
                      <div>
                        ...
                        {href ? (
                          <a href={href} className={styles.contactInfoValue} style={{ color: 'var(--color-white-dim)' }}>
                            {value}
                          </a>
                        ) : (
                          <div className={styles.contactInfoValue} style={{ whiteSpace: 'pre-line' }}>
                            {value}
                          </div>
                        )}
                      </div>
                ...
                <div className="magic-separator"><span>✦</span></div>
        ...
        <section className={`section ${styles.faq}`}>
          <div className="container">
            <div className="text-center reveal" ref={useScrollReveal()}>
              ...
              <h2 className="section-title" style={{ fontSize: '2rem' }}>FAQ</h2>
            </div>
            <div className={`grid-2 stagger`} ref={useScrollReveal()}>
              {[
                { q: 'How far in advance should I book?', a: 'For larger events, 3-6 months is ideal. We recommend contacting us as soon as you have a date, as our calendar fills quickly — especially for weekends.' },
                { q: 'Do you travel internationally?', a: 'Absolutely. Magnus has performed in 15+ countries. Travel and accommodation costs are discussed during the booking process.' },
                { q: 'How long is a typical performance?', a: 'Shows range from 30-minute close-up sessions to 2-hour theatrical productions, depending on your event and needs.' },
                { q: 'Can the show be customized?', a: 'Every performance is tailored to your event. Magnus works closely with clients to incorporate themes, branding, and personal touches.' },
              ].map(({ q, a }) => (
